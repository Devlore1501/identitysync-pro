-- =====================================================
-- Add atc_7d (Add to Cart in last 7 days) computation
-- =====================================================
-- This migration fixes the TODO in engine.ts by adding
-- proper computation of add_to_cart events in last 7 days

-- Drop and recreate the function with atc_7d support
...
BEGIN
  ...
  -- =====================================================
  -- 4. ADD TO CART COUNT (last 7 days) - NEW
  -- =====================================================
  SELECT COUNT(*)
  INTO v_atc_count
  FROM events
  WHERE unified_user_id = p_unified_user_id
    ...
    AND (
      ...
      OR event_name = 'Product Added'
      OR event_name = 'Add to Cart'
    )
    AND event_time > NOW() - INTERVAL '7 days';

  v_atc_count := COALESCE(v_atc_count, 0);

  -- =====================================================
  -- 5. TOP CATEGORY (last 30 days)
  -- =====================================================
  SELECT COALESCE(
    properties->>'collection_handle',
    properties->>'category',
  ...
  -- =====================================================
  -- 6. DROP-OFF STAGE (funnel analysis)
  -- =====================================================
  SELECT
  ...
  -- =====================================================
  -- 9. UPDATE ALL COMPUTED TRAITS (including atc_7d)
  -- =====================================================
